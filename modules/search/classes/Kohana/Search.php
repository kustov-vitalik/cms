...
class Kohana_Search {
    ...
    protected function __construct()
    {
        ...
        {
            throw new Kohana_Exception('Could not find index path :path', array('path' => $this->get_index_path()));
        }
        elseif (!is_dir($this->get_index_path()))
        {
            throw new Kohana_Exception('Index path :path is not a directory', array('path' => $this->get_index_path()));
        }
        elseif (!is_writable($this->get_index_path()))
        {
            throw new Kohana_Exception('Index path :path is not writeable', array('path' => $this->get_index_path()));
        }
        ...
        else
        {
            ...
            if ($this->_config['use_russian_stemming_analyser'])
            {
                $analyzer = new Zend_Search_Lucene_Analysis_Analyzer_Common_Utf8_CaseInsensitive();

                $analyzer->addFilter(new Kohana_PHPMorphy_TokenFilter());
    ...
    public function add($item, $create_new = FALSE)
    {
        ...
        $doc->addField(Zend_Search_Lucene_Field::Keyword('type', $item->get_type(), "UTF-8"));

        // index the object's id - to avoid any confusion, we call it 'identifier' as Lucene uses 'id' attribute internally.
        $doc->addField(Zend_Search_Lucene_Field::UnIndexed('identifier', $item->get_identifier(), "UTF-8"));

        // index the object type plus identifier - this gives us a unique identifier for later retrieval - e.g. to delete
        $doc->addField(Zend_Search_Lucene_Field::Keyword('uid', $item->get_unique_identifier(), "UTF-8"));
        ...
        foreach ($fields as $field)
        {
            ...
            switch ($field->type)
            {
                case Searchable::KEYWORD :
                    $doc->addField(Zend_Search_Lucene_Field::Keyword($field->name, $value, "UTF-8"));
                    break;

                case Searchable::UNINDEXED :
                    $doc->addField(Zend_Search_Lucene_Field::UnIndexed($field->name, $value, "UTF-8"));
                    break;

                case Searchable::BINARY :
                    $doc->addField(Zend_Search_Lucene_Field::Binary($field->name, $value, "UTF-8"));
                    break;

                case Searchable::TEXT :
                    $doc->addField(Zend_Search_Lucene_Field::Text($field->name, $value, "UTF-8"));
                    break;

                case Searchable::UNSTORED :
                    $doc->addField(Zend_Search_Lucene_Field::UnStored($field->name, $value, "UTF-8"));
                    break;
            }
        }
    ...
    public function remove($item)
    {
        ...
        if (sizeof($hits) == 0)
        {
            Kohana::$log->add(Log::ERROR, 'No index entry found for id ' . $item->get_unique_identifier())->write();
        }
        elseif (sizeof($hits) > 1)
        {
            Kohana::$log->add(Log::ERROR, 'Non-unique Identifier - More than one record was returned')->write();
        }
        elseif (sizeof($hits) == 1)
        {
    ...
    /**
     * Remove an entries from the index
     * 
     * @param type $items
     * @return Search
     */
